...
opencode_select_timeout_command() {
  ...
  # An absolute deadline (OPENCODE_DEADLINE_EPOCH, set by run-with-retry.sh)
  # takes precedence so that retries share one wall-clock budget instead of
  # each attempt getting a fresh timeout.
  if [[ -n "${OPENCODE_DEADLINE_EPOCH:-}" ]] && command -v timeout > /dev/null 2>&1; then
    local budget=$((OPENCODE_DEADLINE_EPOCH - $(date +%s)))
    if ((budget > 0)); then
      OPENCODE_TIMEOUT_COMMAND=(timeout --signal=TERM --kill-after=60 "${budget}")
      return
    fi
  fi
...
opencode_configure_run() {
  ...
  if [[ "${REVIEW_ONLY:-false}" == "true" ]]; then
    ...
    # The probe step's emitted config is generated by the action itself from
    # trusted inputs, so it is re-imported here to carry the selected model,
    # any probe-chain provider registrations, and the runbook instructions
    # into the isolated review run.
    if [[ -n "${OPENCODE_PROBE_CONFIG_FILE:-}" && -f "${OPENCODE_PROBE_CONFIG_FILE}" ]]; then
      OPENCODE_CONFIG_CONTENT="$(< "${OPENCODE_PROBE_CONFIG_FILE}")"
